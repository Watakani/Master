...
class LinearTRegression(Dataset):
    def __init__(self, n_train=10000, n_valid=1000, n_test=1000, dim=100, df=10):
        ...
        y_train = StudentT(self.df, loc=X_train @ beta, scale=sigma)
        y_valid = StudentT(self.df, loc=X_valid @ beta, scale=sigma)
        y_test = StudentT(self.df, loc=X_test @ beta, scale=sigma)
    ...
    def log_lik(self, param, data):
        ...
        mean = beta @ data.T
        log_l = torch.sum(torch.log(1 + 1/self.df * ((y.T - mean)/sigma)**2), axis=1)[:, None]
